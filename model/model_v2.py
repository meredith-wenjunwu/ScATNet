...
class CNN(nn.Module):
    ...
    def __init__(self, configs):
        ...
        for i in range(n_layers):
            ...
            modules_attn.append(layer)
        self.attn_layer = nn.Sequential(*modules_attn)
    ...
    def forward(self, x):
        ...
        '''
        h, w = x['sizes'][0].tolist()
        position_enc = PositionalEncoding(feat_dim, h, w)
        enc_output = position_enc(data)
        data = self.proj_layer(enc_output)
        data = self.attn_layer({'image': data, 'mask':mask})
        out = data['image']
        #mask = data['mask']
...
if __name__ == '__main__':
    ...
    out,_ = layer(x)
    print(out.shape)
    # _, indices = torch.sort(out_vis, dim=-1, descending=True)
    # crops = torch.rand(4, 5, 3, 256, 256)
