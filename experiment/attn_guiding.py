...
class AttnGuideReg(nn.Module):
    def __init__(self, num_heads, loss_type, reduce=True, reduction='mean', *args, **kwargs):
    ...
    def gather_attn_map(self, attn_wts, bs):
        ...
        for s in range(len(attn_wts)):
            ...
            for l in range(len(attn_wts[s])):
                attn = attn_wts[s][l].view(bs, -1, P, P)[:,:self.num_heads,:,:] # B x n_h x P x P
                attn = attn.mean(dim=-2) # B x n_h x P 
                attn_map.append(attn)
            attn_wts_gathered.append(attn_map)
        return attn_wts_gathered

    def compute_frobenius_norm_loss(self, attn_wts, target, bs):
        '''
        ...
        :return loss
        '''
        F_norm = torch.zeros(bs).to(target[0].device)
        for s in range(len(attn_wts)):
            for l in range(len(attn_wts[s])):
                F_norm += ((attn_wts[s][l] - target[s].view(bs,1,-1)) ** 2).sum(dim=(1,2))

        if self.reduce:
            F_norm = F_norm.sum()
        return F_norm
    
    def compute_inclusion_exclusion_loss(self, attn_wts, target, bs):
        '''
    ...
    def compute_loss(self, pred, target, bs):
        if self.loss_type == "Frobenius":
            return self.compute_frobenius_norm_loss(pred, target, bs)
        if self.loss_type == "Inclusion_Exclusion":
            return self.compute_inclusion_exclusion_loss(pred, target, bs)
